...
public class PropertyService {
 ...
 public List<PropertyResponse> nearby(Long companyId,double radiusKm,Double maxRent,String type,String sortBy){
  ...
  for(Property p:properties.findAll()){
   ...
   double d=distance.km(c.getLatitude(),c.getLongitude(),p.getLatitude(),p.getLongitude());
   if(d<=radiusKm) result.add(toResponse(p,d));
  }
  Comparator<PropertyResponse> cmp;
  if("rent".equalsIgnoreCase(sortBy)) cmp=Comparator.comparing(PropertyResponse::getRent);
  else cmp=Comparator.comparing(PropertyResponse::getDistanceKm);
  result.sort(cmp);
  return result;
 }
 ...
 public void tag(Long propertyId,Long companyId){
  ...
  Company c=companies.findById(companyId).orElseThrow(()->new IllegalArgumentException("Company not found"));
  double d=distance.km(p.getLatitude(),p.getLongitude(),c.getLatitude(),c.getLongitude());
  ...
  pc.setDistanceKm(Math.round(d*1000.0)/1000.0); tags.save(pc);
 }
 ...
 public List<TaggedCompanyResponse> nearbyCompanies(Long propertyId,double radiusKm){
  ...
  return companies.findAll().stream()
   .map(c -> toCompanyResponse(c, distance.km(p.getLatitude(), p.getLongitude(), c.getLatitude(), c.getLongitude())))
   .filter(c -> c.getDistanceKm() <= radiusKm)
   .sorted(Comparator.comparing(TaggedCompanyResponse::getDistanceKm))
 ...
 public List<TaggedCompanyResponse> taggedCompanies(Long propertyId){
  ...
  return tags.findByPropertyId(propertyId).stream()
   .sorted(Comparator.comparing(PropertyCompany::getDistanceKm))
 ...
 private TaggedCompanyResponse toCompanyResponse(Company c, Double d){
  return new TaggedCompanyResponse(c.getId(), c.getName(), c.getAddress(), c.getLatitude(), c.getLongitude(), Math.round(d * 1000.0) / 1000.0);
 }
